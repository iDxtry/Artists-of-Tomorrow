...
function fetchGeo(school) {
    return new Promise((resolve) => {
        ...
        const cc = cCodes[school.country];
        const url = `https://nominatim.openstreetmap.org/search?q=${searchName}&countrycodes=${cc}&format=json&limit=1`;
...
async function run() {
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        // Wait 1.1s to respect Nominatim API limits (max 1 req/sec)
        await new Promise(r => setTimeout(r, 1100));
        
        const res = await fetchGeo(school);
        if (res.found) {
            console.log(`[${i+1}/${data.length}] ${school.name} -> FOUND: ${res.lat}, ${res.lng}`);
            foundCount++;
        } else {
            console.log(`[${i+1}/${data.length}] ${school.name} -> NOT FOUND.`);
        }
        updatedData.push(res);
    }
    
    fs.writeFileSync('geocoded_schools_v2.json', JSON.stringify(updatedData, null, 2));
